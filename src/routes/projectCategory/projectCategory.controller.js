const {createProjectCategory, getProjectCategoryById, updateProjectCategory, deleteProjectCategory, getAllProjectCategory, searchProjectCategory} = require('../../models/projectCategory/projectCategory.model');
...
const httpCreateCategory = async (req, res) => {
  try {
    ...
    const categoryId = await createProjectCategory({ title });
...
const httpUpdateCategory = async (req, res) => {
  try {
    ...
    const category = await updateProjectCategory(id, { title });
    if (!category) {
      return res.status(404).json({ message: 'Mini project category not found' });
    }
...
const httpGetAllCategories = async (req, res) => {
  try {
    const categories = await getAllProjectCategory();
...
const httpSearchCategories = async (req, res) => {
  try {
    ...
    const categories = await searchProjectCategory(query);
    return res.status(200).json(categories);
  } catch (err) {
