const {createBlogCategory, getBlogCategoryById, updateBlogCategory, deleteBlogCategory, getAllBlogCategory, searchBlogCategory} = require('../../models/blogCategories/blogCategory.model');
...
const httpCreateCategory = async (req, res) => {
  try {
    ...
    const categoryId = await createBlogCategory({ title });
...
const httpUpdateCategory = async (req, res) => {
  try {
    ...
    const category = await updateBlogCategory(id, { title });
    if (!category) {
      return res.status(404).json({ message: 'Mini project category not found' });
    }
...
const httpSearchCategories = async (req, res) => {
  try {
    ...
    const categories = await searchBlogCategory(query);
    return res.status(200).json(categories);
  } catch (err) {
